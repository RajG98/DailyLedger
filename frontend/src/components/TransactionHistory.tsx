...
interface ITransaction {
	id: string;
	titleDes: string;
	amount: number; // better as number
	timestamp: string;
	type: "INCOME" | "EXPENSE";
}
...
export function TransactionHistory() {
  ...
  useEffect(() => {
		const getTransactions = async () => {
			await axios
				.get(`http://localhost:8080/api/api/${userId}/transactions`, {
					headers: {
                        ...
                        {transaction.isCircle ? (
                          ...
                          <svg width="25" height="25" viewBox="0 0 25 25" fill="none">
                            <path d={transaction.icon} fill={transaction.iconFill} />
									...
									<td className="d-none d-xl-table-cell">{transaction.type}</td>
								</tr>
							))}
						</tbody>
					</table>
